 ...
 * sub-path of github.io, not the final domain, and the share card has to be
 * addressed there or the preview comes up blank when the link is pasted.
 * SITE_URL must end in a slash so relative metadata resolves inside the path.
 */
const siteUrl = process.env.SITE_URL ?? site.brand.domain;

/* Generated by `npm run og` and committed, rather than by Next's
   opengraph-image convention: that convention emits a file with no extension,
   which GitHub Pages will not serve. Relative so it resolves under any
   basePath. */
